...
const aggregate = (entries: AnalyticsEntry[]) => {
  ...
  const uniqueSessions7d = new Set(pageviews.filter((e) => e.ts >= since7d && e.sessionId).map((e) => e.sessionId)).size;
  const uniqueSessions30d = new Set(pageviews.filter((e) => e.ts >= since30d && e.sessionId).map((e) => e.sessionId)).size;
  const uniqueSessionsTotal = new Set(pageviews.filter((e) => e.sessionId).map((e) => e.sessionId)).size;
  ...
  const topPages = Array.from(pageCounts.entries()).map(([page, counts]) => ({ page, ...counts })).sort((a, b) => b.count - a.count).slice(0, 20);
  ...
  const topCountries = Array.from(countryCounts.entries()).map(([country, counts]) => ({ country, ...counts })).sort((a, b) => b.count - a.count).slice(0, 30);
  ...
  const topUtmSources = Array.from(utmCounts.values()).sort((a, b) => b.count - a.count).slice(0, 20);
  ...
  const embeddedBrowsers = Array.from(embeddedCounts.entries()).map(([browser, counts]) => ({ browser, ...counts })).sort((a, b) => b.count - a.count);
  ...
  for (const e of eventEntries) {
    ...
    if (!existing) { existing = { count7d: 0, count30d: 0, count: 0, metadata: new Map() }; eventMap.set(e.event, existing); }
    if (e.ts >= since7d) existing.count7d++;
    if (e.ts >= since30d) existing.count30d++;
    existing.count++;
    if (e.eventData) {
      for (const [key, value] of Object.entries(e.eventData)) {
        ...
        if (!keyMap) { keyMap = new Map(); existing.metadata.set(key, keyMap); }
  ...
  const events = Array.from(eventMap.entries()).map(([event, data]) => {
    ...
    for (const [key, valMap] of data.metadata) {
      metadata[key] = Array.from(valMap.entries()).map(([value, count]) => ({ value, count })).sort((a, b) => b.count - a.count);
    }
  ...
  return {
    ...
    topPages, topCountries, topUtmSources, embeddedBrowsers, events,
  };
};
